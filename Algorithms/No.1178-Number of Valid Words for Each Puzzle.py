...
解题思路：
    ...
    第一步 状态压缩：
        为了解决这个问题，用二进制数字表示每个 word 和 puzzle
        26个小写字母，压缩到一个int中。int中的每一位取0和1表示字符是否出现过
        比如："aabb" -> 11，"accc" -> 101
        把每个word都做状态压缩，并用hashmp保存每个二进制数字出现的次数
    第二步 匹配：
        根据题目要求，word 状态压缩后的数字和 puzzle[0] + subset(puzzle[1:N-1])状态压缩后的数字相等
        subset 用 No.78 子集方法 暴力求出 puzzle[1:N-1] 的所有子集，
